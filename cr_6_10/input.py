...
jw_nelec = (6,0)
nelec_f = ((3,0),(3,0))
las = LASSCF (mf, (5,5), ((3,0),(3,0)),spin_sub=(4,4))
#frag_atom_list = [[0],[1]]
...
las.kernel(mo)
print("LASSCF HS energy: ", las.e_tot)
tools.molden.from_mo(mol,'as_increase_crcr_las_hs.molden',las.mo_coeff)
np.save('kd_6_10_lasmo_hs',las.mo_coeff)
#np.save('kf_6_10_lasci', las.ci) 
ncore = las.ncore
ncas = las.ncas
ncas_sub = las.ncas_sub
# CASCI h1 & h2 for VQE Hamiltonian
mc = mcscf.CASCI(mf,10,(6,0))
...
mc.kernel()
print('CASCI with LAS orbital HS', mc.e_tot)
...
ucj3 = LUCJ_circuit(10,6,0,cas_h1e,eri)

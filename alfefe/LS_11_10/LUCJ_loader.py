...
def LUCJ_load(norb, nelec_a, nelec_b, hcore, eri,las_ci):
    ...
    mol_MO = gto.M()
    mol_MO.symmetry = False
    mol_MO.nelectron = nelec_a + nelec_b
    mol_MO.spin = np.abs(nelec_a-nelec_b)
    #mol_MO.nao_nr = lambda *args, **kwargs: norb
    mol_MO.max_memory=1400000
    mol_MO.nao = norb
    mol_MO.max_cycle =200
    ...
    if nelec_a != nelec_b:
        ...
        result = scipy.optimize.minimize(fun, x0 =operator.to_parameters(interaction_pairs=interaction_pairs),method="L-BFGS-B")
    ...
    circuit.append(ffsim.qiskit.PrepareHartreeFockJW(int(norb), nelec), qubits)
